                ...
                for ech_tmstamp_obj in ech_chnl_obj.timestampobject:
                    ...
                    for evry_attrib in ech_tmstamp_obj.CurrentAttr:
                        ...
                        if tracker_name == 'Re3':
                            ...
                            print("manually_corrected ::>", evry_attrib.hasUserCorrected, manually_corrected)
                            box_lst = []
                            ...
                            global PREV_IMAGE
                            if evry_attrib.hasUserCorrected == 1:
                            ...
                            elif evry_attrib.hasUserCorrected == 2:
                                bbox, trackeddata = self.tracker.track(trackid, np.array(imageRGB))
                                bbox = [int(evry_ordinate) for evry_ordinate in bbox]

                            elif evry_attrib.hasUserCorrected == 3:
                                # if trackid not in self.trkid_lst:
                                bbox, trackeddata = self.tracker.track(trackid, np.array(PREV_IMAGE),
                                                                       bbox_coord_lst)
                                bbox, trackeddata = self.tracker.track(trackid, np.array(imageRGB))
                                bbox = [int(evry_ordinate) for evry_ordinate in bbox]
                            else:
                                bbox, trackeddata = self.tracker.track(trackid, np.array(imageRGB))
                                bbox = [int(evry_ordinate) for evry_ordinate in bbox]
                            print("tracked box >>", trackid, bbox)
                            print("\n")
                            PREV_IMAGE = imageRGB
