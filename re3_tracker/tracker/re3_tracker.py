...
class Re3Tracker(object):
    def __init__(self, gpu_id=GPU_ID):
        ...
        self.conv_layers = network.returnConvLayers(self.imagePlaceholder, num_unrolls=1, batch_size=self.batch_size,
                                                    train=False,
                                                    prevLstmState=self.prevLstmState)
    ...
    def track(self, unique_id, image, starting_box=None):
        ...
        if starting_box is not None:
            ...
            lstmState, pastBBox, prevImage, originalFeatures, forwardCount,convFeatures = self.tracked_data[unique_id]
        else:
        ...
        rawOutput, s1, s2, convFeatures = self.sess.run([self.outputs, self.state1, self.state2, self.conv_layers1], feed_dict=feed_dict)
        ...
        self.tracked_data[unique_id] = (lstmState, outputBox, image, originalFeatures, forwardCount,convFeatures1)
        end_time = time.time()
    ...
    def multi_track(self, unique_ids, image, starting_boxes=None):
        ...
        for unique_id in unique_ids:
            if unique_id in starting_boxes:
                ...
                forwardCount = 0
                convFeatures = None
                self.tracked_data[unique_id] = (lstmState, pastBBox, image, originalFeatures, forwardCount, convFeatures)
            elif unique_id in self.tracked_data:
                lstmState, pastBBox, prevImage, originalFeatures, forwardCount, convFeatures = self.tracked_data[unique_id]
            else:
                raise Exception('Unique_id %s with no initial bounding box' % unique_id)
            ...
            # cv2.imwrite('%d%d.jpg'%(int(unique_id),forwardCount),croppedInput0)
            # cv2.imwrite('%d_%d.jpg'%(int(unique_id),forwardCount), croppedInput1)
        ...
        for uu,unique_id in enumerate(unique_ids):
            ...
            if forwardCount > 0 and forwardCount % MAX_TRACK_LENGTH == 0:
                ...
                _, s1_new, s2_new,_ = self.sess.run([self.outputs, self.state1, self.state2, self.conv_layers1], feed_dict=feed_dict)
                convFeatures1 = self.sess.run([self.conv_layers], feed_dict=feed_dict)
    ...
    def returnConvLayers(self, bbox, image, starting_boxes=None):
        ...
        croppedInput0, pastBBoxPadded = im_util.get_cropped_input(image, bbox, CROP_PAD, CROP_SIZE)
