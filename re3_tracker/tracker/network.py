...
def inference(inputs, num_unrolls, train, batch_size=None, prevLstmState=None, reuse=None):
    ...
    with tf.variable_scope('re3', reuse=reuse):
        conv_layers,conv5_skip_flat = alexnet_conv_layers(inputs, batch_size, num_unrolls)
    ...
    if prevLstmState is not None:
        return fc_output_out, state1, state2, conv5_skip_flat
    else:
        return fc_output_out, conv5_skip_flat

def returnConvLayers(inputs, num_unrolls, train, batch_size=None, prevLstmState=None, reuse=None):
    # Data should be in order BxTx2xHxWxC where T is the number of unrolls
    # Mean subtraction
    if batch_size is None:
        batch_size = int(inputs.get_shape().as_list()[0] / (num_unrolls * 2))
    ...
    _,conv5_skip_flat = alexnet_conv_layers(inputs, batch_size, num_unrolls)
    if prevLstmState is not None:
        return conv5_skip_flat
    else:
        return conv5_skip_flat
